...
class Clinical_finalization_service {
    ...
    public function finalize($requestId,$userId,$key){$requestId=(int)$requestId;$userId=(int)$userId;$key=trim((string)$key);if($requestId<1||$userId<1||$key===''||strlen($key)>191)return $this->fail('INVALID_FINALIZATION_INPUT');
        ...
        try {
            ...
            if(!doclinc_append_request_event($requestId,'clinical_record.finalized',$event,get_instance())){$conflict=$this->model->eventByKeyForUpdate($eventKey);if($conflict){$meta=json_decode((string)$conflict->metadata_json,true)?:[];if((int)$conflict->request_id!==$requestId||(int)($meta['finalized_by_user_id']??0)!==$userId)return $this->rollback('FINALIZATION_KEY_CONFLICT');$rid=(int)($meta['record_id']??0);$saved=$rid?$this->db->where('record_id',$rid)->get('medicalrecords')->row():null;if($saved){$this->db->trans_commit();return ['status'=>'success','record_id'=>$rid,'idempotent'=>true,'medicalrecord'=>$saved];}}return $this->rollback('WRITE_FAILED');}
