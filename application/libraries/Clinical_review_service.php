...
class Clinical_review_service
{
    ...
    public function review($requestId, $visitResultId, $reviewerUserId, $decision, $correctionReason, $reviewNotes, $idempotencyKey)
    {
        ...
        try {
            ...
            if($id<1){$error=$this->db->error();$code=(int)($error['code']??0);if($code===0&&isset($this->db->conn_id)&&function_exists('mysqli_errno'))$code=(int)mysqli_errno($this->db->conn_id);$duplicate=in_array($code,array(1062,1213),true);if($duplicate){$this->db->trans_rollback();if($this->db->trans_begin()){$existing=$this->model->getByIdempotencyKeyForUpdate($key);if($existing)return $this->replayOrConflict($existing,$requestId,$visitResultId,$reviewerUserId,$decision,$reason,$notes,$rdOk,$performerOk);$existingResult=$this->model->getForResultForUpdate($visitResultId);if($existingResult)return $this->rollback('REVIEW_ALREADY_EXISTS');$this->db->trans_rollback();}}return $this->fail('WRITE_FAILED');}
        ...
        } catch(Throwable $e){$error=$this->db->error();$code=(int)($error['code']??0);if($code===0&&isset($this->db->conn_id)&&function_exists('mysqli_errno'))$code=(int)mysqli_errno($this->db->conn_id);$duplicate=in_array($code,array(1062,1213),true)||in_array((int)$e->getCode(),array(1062,1213),true);$this->db->trans_rollback();if($duplicate&&$this->db->trans_begin()){$existing=$this->model->getByIdempotencyKeyForUpdate($key);if($existing)return $this->replayOrConflict($existing,$requestId,$visitResultId,$reviewerUserId,$decision,$reason,$notes,false,false);$existingResult=$this->model->getForResultForUpdate($visitResultId);if($existingResult)return $this->rollback('REVIEW_ALREADY_EXISTS');$this->db->trans_rollback();}return $this->fail('WRITE_FAILED');}
    }
