...
if (table_exists($db, 'consultation_messages')) {
	echo "[skip] table exists `consultation_messages`\n";
	exit(0);
}

run_query($db, "CREATE TABLE `consultation_messages` (
	...
	`attachment_original_name` varchar(180) NULL,
	`is_read` tinyint(1) NOT NULL DEFAULT 0,
	`created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
	`deleted_at` datetime NULL,
	PRIMARY KEY (`message_id`),
	KEY `idx_consultation_messages_request_created` (`request_id`, `created_at`),
	KEY `idx_consultation_messages_request_id` (`request_id`, `message_id`),
	KEY `idx_consultation_messages_sender_created` (`sender_user_id`, `created_at`),
	KEY `idx_consultation_messages_read` (`is_read`),
	KEY `idx_consultation_messages_deleted` (`deleted_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4", "created table `consultation_messages`");
