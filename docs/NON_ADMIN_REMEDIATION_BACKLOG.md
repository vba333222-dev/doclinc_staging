...
- Harness MariaDB disposable kini menjalankan query builder CI3 production terhadap dua tenant dan mengunci exact count, cross-tenant denial, payload least-privilege, schema denial, serta zero-write. Overflow staf, request, dan assignment semuanya fail-closed; staf tidak lagi boleh terpotong diam-diam pada batas snapshot. Harness masih harus dieksekusi pada PHP 8.1 + MariaDB 10.11 sebelum aktivasi.
...
- PUSKESMAS-01 batch operasional implemented locally: dedicated command-center route/navigation, exact canonical actor policy, bounded tenant snapshot tanpa silent truncation, workload/presence/visit aggregation, assignment exception board, personal-Nakes denial, serta harness MariaDB disposable dua tenant. Fitur tetap default off sampai harness dijalankan pada PHP 8.1/MariaDB 10.11, cross-tenant HTTP test, serta authenticated UAT selesai.
...
- Production-readiness reporting is split into managed-data, controlled-enforcement, and strict production-activation gates. Password-change denials, invalid identities, self-service gaps, and Dinkes-managed Puskesmas/staff gaps are counted separately without printing PII. Feature activation remains a separate reviewed rollout and must not infer or fabricate NIK/KK/BPJS/NIP/SIP/address data.
- Admin Dinas Kesehatan remediation queue exposes per-record readiness reasons and bounded filters for active Puskesmas/staff. Inactive records are excluded from readiness totals; NIP, SIP, facility address, and account links remain explicit human-managed edits with no bulk autofill.
- Nakes/command-center temporary-password lifecycle is fail-closed: account creation and Admin reset explicitly set the first-login gate; registration, temporary password, reset, and first-login replacement all require 8–72 characters with uppercase, lowercase, number, and special character; and the Admin queue identifies linked Nakes who have not completed the required password change. Existing 24 password-change rows remain user-owned remediation and are never auto-cleared.
- Profile remediation hardening now uses one private self-upload boundary for Warga, personal Nakes, and command-center Puskesmas. The global role-prerequisite gate remains environment-controlled; enabling it intentionally blocks every incomplete actor except exact remediation/status/photo/logout routes until self-service and Dinkes-managed data are complete.
- Nakes credential readiness no longer trusts `must_change_password=0` by itself. An active Nakes/Puskesmas account without valid `password_changed_at` evidence is treated as never activated and remains locked. Admin reset preserves prior user-change evidence, the Dinkes queue separates first activation from post-reset waiting, and personal-account reset is one-account-at-a-time with a unique temporary password, row locks, audit logging, and no plaintext replay or bulk reset.
- Dokumen ini belum menyatakan remediation selesai.
# 2026-08-03 role-aware dashboard and communication remediation

- Puskesmas command-center identity is facility-based; personal demographics and a personal profile photo are no longer prerequisites.
- Puskesmas contact email/phone remain self-service, while facility master data stays under Dinas Kesehatan ownership and operational readiness warnings remain non-blocking.
- Duplicate Nakes status and staff cards are consolidated into one roster with authorized thumbnails and online/offline state.
- Command-center chat is read-only in both authorization and UI; patients and personal Nakes remain the only chat writers.
- Personal profile photos are rendered on both dashboards and chat messages through the authorized private-media route.
- Patient hero disabled-state copy receives explicit high-contrast styling.
- Notification audio defaults on and the page-level enable/disable button is removed; playback still follows browser trusted-interaction and background-execution policy.
- Patient dashboard and consultation creation now share one authenticated reverse-geocoding boundary. Coordinates travel in a CSRF-protected POST body, resolved addresses are cached privately and attributed, provider access is globally throttled and runtime-switchable, and consultation requests persist real address text instead of system fallback wording.
