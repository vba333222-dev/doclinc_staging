...
- Nakes/command-center temporary-password lifecycle is fail-closed: account creation and Admin reset explicitly set the first-login gate, password input is bounded to 10–72 characters, and the Admin queue identifies linked Nakes who have not completed the required password change. Existing 24 password-change rows remain user-owned remediation and are never auto-cleared.
