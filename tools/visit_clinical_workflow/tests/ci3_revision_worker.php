...
$role = (string)($argv[1] ?? 'A'); $barrier = (string)($argv[2] ?? ''); $announceFile = (string)($argv[8] ?? '');
...
if ($announceFile !== '') { file_put_contents($announceFile, json_encode(array('pid'=>getmypid(),'connection_id'=>$conn ? (int)$conn->id : 0)), LOCK_EX); }
