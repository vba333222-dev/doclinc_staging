...
class ClinicalSchemaTestRunner
{
	...
	private function runUnitTests()
	{
		...
		$audit = $loader->loadById('20260722000100_clinical_metadata_registration_audit_foundation');
		...
		$this->test('345_metadata_registration_audit_descriptor_parses',function()use($audit){$this->assertSame('20260722000100_clinical_metadata_registration_audit_foundation',$audit['migration_id']);$this->assertSame(array('clinical_master_metadata_registration_events'),$audit['expected_created_tables']);$this->assertSame(array(),$audit['expected_modified_tables']);$this->assertSame(1,count($audit['steps']));});
		$this->test('346_metadata_registration_audit_identifier_is_catalog_maximum',function()use($loader,$audit){$ids=array_keys($loader->loadCatalog());$this->assertSame($audit['migration_id'],end($ids));foreach(array_slice($ids,0,-1) as $id){$this->assertTrue(strcmp($id,$audit['migration_id'])<0);}});
		$this->test('347_metadata_registration_audit_checksum_is_exact_and_deterministic',function()use($loader,$audit){$this->assertSame('9af9d7fd42e2e17908ac162c89d0ea7a062b5aeb0366d9ae15ecf82c5a8e2c95',$audit['checksum']);$this->assertSame($audit['checksum'],$loader->loadById($audit['migration_id'])['checksum']);});
		$this->test('348_metadata_registration_audit_dependencies_are_all_existing_migrations',function()use($audit){$expected=array('00000000000000_clinical_schema_ledger','20260718000100_clinical_import_audit_foundation','20260719000100_clinical_master_provenance_foundation','20260720000100_clinical_record_reference_foundation','20260721000100_clinical_package_registration_prerequisites');$this->assertSame($expected,array_column($audit['required_applied_migrations'],'migration_id'));});
		$this->test('349_metadata_registration_audit_declared_table_matches_sql',function()use($audit){$step=$audit['steps'][0];$this->assertSame('create_table',$step['type']);$this->assertSame('clinical_master_metadata_registration_events',$step['expected_schema']['table_name']);$this->assertSame(1,preg_match_all('/\\bCREATE\\s+TABLE\\s+`clinical_master_metadata_registration_events`/i',$step['sql_bytes'],$matches));});
		$this->test('350_metadata_registration_audit_columns_are_exact',function()use($audit){$schema=$audit['steps'][0]['expected_schema'];$expected=array('metadata_registration_event_id','operation_type','execution_mode','registration_reference','package_key','package_version','package_checksum','package_checksum_profile','package_snapshot_sha256','metadata_entity_count','metadata_rows_planned','metadata_rows_inserted','state_before','state_after','result','idempotent_noop','metadata_transaction_committed','safe_failure_code','occurred_at');$this->assertSame($expected,array_column($schema['columns'],'name'));$nullable=array();foreach($schema['columns'] as $column){if($column['nullable']){$nullable[]=$column['name'];}$this->assertSame('',$column['generation_expression']);$this->assertSame(1,preg_match_all('/^\\s*`'.preg_quote($column['name'],'/').'`\\s+(?:BIGINT|VARCHAR|CHAR|SMALLINT|TINYINT|DATETIME)\\b/im',$audit['steps'][0]['sql_bytes'],$matches));}$this->assertSame(array('state_before','state_after','safe_failure_code'),$nullable);$this->assertSame(array('metadata_registration_event_id'),$schema['primary_key']);});
		$this->test('351_metadata_registration_audit_indexes_are_exact',function()use($audit){$schema=$audit['steps'][0]['expected_schema'];$expected=array(array('name'=>'idx_clinical_metadata_registration_reference','columns'=>array('registration_reference')),array('name'=>'idx_clinical_metadata_registration_package','columns'=>array('package_key','package_version','package_checksum')),array('name'=>'idx_clinical_metadata_registration_snapshot','columns'=>array('package_snapshot_sha256')),array('name'=>'idx_clinical_metadata_registration_occurred','columns'=>array('occurred_at')),array('name'=>'idx_clinical_metadata_registration_result_occurred','columns'=>array('result','occurred_at')));$this->assertSame(array(),$schema['unique_indexes']);$this->assertSame($expected,$schema['indexes']);foreach($expected as $index){$this->assertSame(1,substr_count($audit['steps'][0]['sql_bytes'],'KEY `'.$index['name'].'`'));}});
		$this->test('352_metadata_registration_audit_checks_are_exact',function()use($audit){$expected=array('chk_clinical_metadata_registration_operation','chk_clinical_metadata_registration_execution_mode','chk_clinical_metadata_registration_checksum','chk_clinical_metadata_registration_snapshot','chk_clinical_metadata_registration_checksum_profile','chk_clinical_metadata_registration_entity_count','chk_clinical_metadata_registration_rows_planned','chk_clinical_metadata_registration_rows_inserted','chk_clinical_metadata_registration_noop_boolean','chk_clinical_metadata_registration_committed_boolean','chk_clinical_metadata_registration_state_before','chk_clinical_metadata_registration_state_after','chk_clinical_metadata_registration_result','chk_clinical_metadata_registration_result_consistency','chk_clinical_metadata_registration_reference','chk_clinical_metadata_registration_package_identity','chk_clinical_metadata_registration_failure_code');$checks=$audit['steps'][0]['expected_schema']['check_constraints'];$this->assertSame($expected,array_column($checks,'name'));foreach($expected as $name){$this->assertSame(1,substr_count($audit['steps'][0]['sql_bytes'],'CONSTRAINT `'.$name.'` CHECK'));}});
		$this->test('353_metadata_registration_audit_sql_contains_no_drop',function()use($audit){$this->assertSame(0,preg_match_all('/\\bDROP\\b/i',$audit['steps'][0]['sql_bytes'],$matches));});
		$this->test('354_metadata_registration_audit_sql_contains_no_update',function()use($audit){$this->assertSame(0,preg_match_all('/\\bUPDATE\\b/i',$audit['steps'][0]['sql_bytes'],$matches));});
		$this->test('355_metadata_registration_audit_sql_contains_no_delete',function()use($audit){$this->assertSame(0,preg_match_all('/\\bDELETE\\b/i',$audit['steps'][0]['sql_bytes'],$matches));});
		$this->test('356_metadata_registration_audit_imports_no_records',function()use($audit){$sql=$audit['steps'][0]['sql_bytes'];$this->assertSame(false,$audit['imports_package_data']);$this->assertTrue(preg_match('/\\b(?:INSERT|REPLACE|LOAD\\s+DATA)\\b/i',$sql)!==1);$this->assertTrue(stripos($sql,'clinical_master_record_revisions')===false&&stripos($sql,'database/master_data')===false);});
		$this->test('357_metadata_registration_audit_enables_no_runtime',function()use($audit){$this->assertSame(false,$audit['enables_runtime']);$this->assertTrue(stripos($audit['steps'][0]['sql_bytes'],'runtime_enabled')===false&&stripos($audit['steps'][0]['sql_bytes'],'source_runtime')===false);});
		$this->test('358_metadata_registration_audit_does_not_modify_existing_tables',function()use($audit){$plan=(new ClinicalSchemaMigrator(new ClinicalSchemaDescriptor($this->migrationRoot),null,$this->config))->plan($audit);$this->assertSame(array(),$plan['modified_tables']);$this->assertSame(array('clinical_master_metadata_registration_events'),$plan['created_tables']);$this->assertSame(false,$plan['ddl_executed']);$this->assertSame(0,preg_match_all('/\\bALTER\\s+TABLE\\b/i',$audit['steps'][0]['sql_bytes'],$matches));});
		$this->test('359_metadata_registration_audit_has_no_foreign_keys_or_triggers',function()use($audit){$schema=$audit['steps'][0]['expected_schema'];$this->assertSame(array(),$schema['foreign_keys']);$this->assertTrue(preg_match('/\\b(?:REFERENCES|FOREIGN\\s+KEY|TRIGGER)\\b/i',$audit['steps'][0]['sql_bytes'])!==1);});
		$this->test('360_metadata_registration_audit_apply_is_idempotent',function()use($loader,$ledger,$foundation,$provenance,$reference,$prerequisite,$audit){$fake=$this->fakeAuditBase($ledger,$foundation,$provenance,$reference,$prerequisite,$audit);$migrator=new ClinicalSchemaMigrator($loader,$fake,$this->config);$first=$migrator->execute('apply',$audit,$this->writeOptions());$this->assertSame(true,$first['ddl_executed']);$this->assertSame(false,$first['already_applied']);$ddl=$fake->ddlCount;$ledgerRows=count($fake->ledger);$second=$migrator->execute('apply',$audit,$this->writeOptions());$this->assertSame(false,$second['ddl_executed']);$this->assertSame(true,$second['already_applied']);$this->assertSame($ddl,$fake->ddlCount);$this->assertSame($ledgerRows,count($fake->ledger));});
		$this->test('361_metadata_registration_audit_verify_detects_missing_table',function()use($loader,$ledger,$foundation,$provenance,$reference,$prerequisite,$audit){$fake=$this->fakeAuditApplied($loader,$ledger,$foundation,$provenance,$reference,$prerequisite,$audit);unset($fake->present['clinical_master_metadata_registration_events']);$this->expectCode(function()use($loader,$fake,$audit){(new ClinicalSchemaMigrator($loader,$fake,$this->config))->verify($audit,$this->writeOptions());},'migration_schema_mismatch');});
		$this->test('362_metadata_registration_audit_verify_detects_missing_column',function()use($loader,$ledger,$foundation,$provenance,$reference,$prerequisite,$audit){$fake=$this->fakeAuditApplied($loader,$ledger,$foundation,$provenance,$reference,$prerequisite,$audit);array_splice($fake->schemas['clinical_master_metadata_registration_events']['columns'],5,1);$this->expectCode(function()use($loader,$fake,$audit){(new ClinicalSchemaMigrator($loader,$fake,$this->config))->verify($audit,$this->writeOptions());},'migration_schema_mismatch');});
		$this->test('363_metadata_registration_audit_verify_detects_wrong_column_type',function()use($loader,$ledger,$foundation,$provenance,$reference,$prerequisite,$audit){$fake=$this->fakeAuditApplied($loader,$ledger,$foundation,$provenance,$reference,$prerequisite,$audit);$fake->schemas['clinical_master_metadata_registration_events']['columns'][9]['column_type']='int(10) unsigned';$this->expectCode(function()use($loader,$fake,$audit){(new ClinicalSchemaMigrator($loader,$fake,$this->config))->verify($audit,$this->writeOptions());},'migration_schema_mismatch');});
		$this->test('364_metadata_registration_audit_verify_detects_missing_index',function()use($loader,$ledger,$foundation,$provenance,$reference,$prerequisite,$audit){$fake=$this->fakeAuditApplied($loader,$ledger,$foundation,$provenance,$reference,$prerequisite,$audit);array_pop($fake->schemas['clinical_master_metadata_registration_events']['indexes']);$this->expectCode(function()use($loader,$fake,$audit){(new ClinicalSchemaMigrator($loader,$fake,$this->config))->verify($audit,$this->writeOptions());},'migration_schema_mismatch');});
		$this->test('365_metadata_registration_audit_verify_detects_missing_check',function()use($loader,$ledger,$foundation,$provenance,$reference,$prerequisite,$audit){$fake=$this->fakeAuditApplied($loader,$ledger,$foundation,$provenance,$reference,$prerequisite,$audit);array_pop($fake->schemas['clinical_master_metadata_registration_events']['check_constraints']);$this->expectCode(function()use($loader,$fake,$audit){(new ClinicalSchemaMigrator($loader,$fake,$this->config))->verify($audit,$this->writeOptions());},'migration_schema_mismatch');});
		$this->test('366_metadata_registration_audit_verify_detects_unexpected_foreign_key',function()use($loader,$ledger,$foundation,$provenance,$reference,$prerequisite,$audit){$fake=$this->fakeAuditApplied($loader,$ledger,$foundation,$provenance,$reference,$prerequisite,$audit);$fake->schemas['clinical_master_metadata_registration_events']['foreign_keys'][]=array('name'=>'fk_unexpected_audit_package','columns'=>array('package_key'),'referenced_table'=>'clinical_master_packages','referenced_columns'=>array('package_key'),'on_update'=>'RESTRICT','on_delete'=>'RESTRICT');$this->expectCode(function()use($loader,$fake,$audit){(new ClinicalSchemaMigrator($loader,$fake,$this->config))->verify($audit,$this->writeOptions());},'migration_schema_mismatch');});
		$this->test('367_metadata_registration_audit_is_append_only_by_schema',function()use($audit){$schema=$audit['steps'][0]['expected_schema'];$this->assertTrue(!$this->hasColumn($schema,'updated_at')&&!$this->hasColumn($schema,'deleted_at'));$this->assertSame(array(),$schema['unique_indexes']);foreach($schema['columns'] as $column){$this->assertTrue(stripos($column['extra'],'on update')===false&&$column['generation_expression']==='');}});
	}
	...
	private function fakeAuditBase(array $ledger, array $foundation, array $provenance, array $reference, array $prerequisite, array $audit)
	{
		$fake=$this->fakePrerequisiteApplied($ledger,$foundation,$provenance,$reference,$prerequisite);
		$fake->addDescriptorSchemas($audit);
		return $fake;
	}

	private function fakeAuditApplied(ClinicalSchemaDescriptor $loader,array $ledger,array $foundation,array $provenance,array $reference,array $prerequisite,array $audit)
	{
		$fake=$this->fakeAuditBase($ledger,$foundation,$provenance,$reference,$prerequisite,$audit);
		(new ClinicalSchemaMigrator($loader,$fake,$this->config))->execute('apply',$audit,$this->writeOptions());
		return $fake;
	}
	...
	{
		$this->test($name,function()use($mutation,$expectedCode){$root=$this->fixture();try{@unlink($root.'/20260722000100_clinical_metadata_registration_audit_foundation.json');$mutation($root);$loader=new ClinicalSchemaDescriptor($root);$descriptor=$loader->loadById('20260721000100_clinical_package_registration_prerequisites');$migrator=new ClinicalSchemaMigrator($loader,null,$this->config);$this->expectCode(function()use($migrator,$descriptor){$migrator->plan($descriptor);},$expectedCode);}finally{$this->removeTree($root);}});
	}
	...
	private function addFutureDatasetMutationDescriptor($root, $fork)
	{
		...
		$futureId='20260723000100_test_dataset_mutation';
