...
expect(adminController.includes('strlen($password) < 10 || strlen($password) > 72')
  && adminView.includes('wajib membuat password baru saat login pertama')
  && (adminView.match(/minlength="10" maxlength="72"/g) || []).length === 2, 'personal_temporary_password_contract_is_explicit');
...
expect(commandCenterController.includes('required|min_length[10]|max_length[72]')
  && commandCenterController.includes('strlen($password) < 10 || strlen($password) > 72')
  && (commandCenterView.match(/minlength="10" maxlength="72"/g) || []).length === 4
