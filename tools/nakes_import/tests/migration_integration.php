...
function migration_fixture(mysqli $db, $staffIdType = 'int(10) unsigned')
{
	$allowedStaffIdTypes = array('int(10) unsigned', 'int(11)', 'int(10)');
	if (!in_array($staffIdType, $allowedStaffIdTypes, true)) throw new RuntimeException('staff_id_fixture_type');
	...
	$db->query("CREATE TABLE puskesmas_staff (staff_id $staffIdType NOT NULL AUTO_INCREMENT,kode_pkm varchar(100) NOT NULL,nama varchar(150) NOT NULL,no_hp varchar(50) NULL,profesi varchar(100) NULL,nomor_sip varchar(100) NULL,user_id int(11) NULL,status enum('aktif','nonaktif') NOT NULL DEFAULT 'aktif',created_at datetime NOT NULL,updated_at datetime NULL,created_by_user_id int(11) NULL,updated_by_user_id int(11) NULL,PRIMARY KEY(staff_id)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
...
try{
	...
	$stage='plan';list($code,$out)=migration_run($migration,array());migration_expect($code===0&&strpos($out,'DDL_EXECUTED=false')!==false&&!$db->query("SHOW COLUMNS FROM users LIKE 'must_change_password'")->fetch_assoc()&&!$db->query("SHOW COLUMNS FROM users LIKE 'password_changed_at'")->fetch_assoc()&&!$db->query("SHOW COLUMNS FROM puskesmas_staff LIKE 'penugasan'")->fetch_assoc(),'plan_zero_write');
	...
	$type=$db->query("SELECT COLUMN_TYPE FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='puskesmas_staff' AND COLUMN_NAME='staff_id'")->fetch_assoc();migration_expect($type&&strtolower((string)$type['COLUMN_TYPE'])==='int(10) unsigned','unsigned_staff_id_accepted');
	...
	$stage='int11_rejected';migration_fixture($db,'int(11)');migration_writer($db,$database);list($code,$out,$err)=migration_run($migration,$args);migration_expect($code!==0&&strpos($err,'SAFE_ERROR_CODE=base_schema_mismatch')!==false&&!$db->query("SHOW COLUMNS FROM users LIKE 'must_change_password'")->fetch_assoc()&&!$db->query("SHOW COLUMNS FROM puskesmas_staff LIKE 'penugasan'")->fetch_assoc(),'int11_rejected');
	$stage='signed_int10_rejected';migration_fixture($db,'int(10)');migration_writer($db,$database);list($code,$out,$err)=migration_run($migration,$args);migration_expect($code!==0&&strpos($err,'SAFE_ERROR_CODE=base_schema_mismatch')!==false&&!$db->query("SHOW COLUMNS FROM users LIKE 'must_change_password'")->fetch_assoc()&&!$db->query("SHOW COLUMNS FROM puskesmas_staff LIKE 'penugasan'")->fetch_assoc(),'signed_int10_rejected');
