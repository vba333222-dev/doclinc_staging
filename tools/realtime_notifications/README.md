...
The existing 30-second polling remains available. Realtime publications and recovery gaps refresh `/notifications/snapshot`; payload data is never treated as notification state. The sound control requires a user gesture before Web Audio can run. Initial notifications are silent, and a bounded browser-side notification ID history prevents repeated chimes after reconnect, recovery, or polling.

Foreground sound applies only while the page is open. Closed-browser and native background delivery are not provided by this integration.
